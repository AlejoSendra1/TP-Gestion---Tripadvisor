...
public class ReviewService {
     ...
     * userRepository.save(traveler);
     * System.out.println("XP removido de " + traveler.getEmail() + ": " +
     * xpToRemove + " puntos");
     * }
     */
    ...
    public Page<ReviewHistoryRegisterDTO> getUserReviews(Pageable pageable) {
        Page<Review> userReviews = reviewRepository.findByReviewerId(userService.getCurrentAuthenticatedUser().getId(),
                pageable);
    ...
    private ReviewHistoryRegisterDTO mapToReviewHistoryRegisterDTO(Review review) {
        ...
        Long qualification = this.qualificationRepository.countPositiveQualifByReviewId(review.getId())
                - this.qualificationRepository.countNegativeQualifByReviewId(review.getId());
